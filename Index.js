...
// ========== 远程上下文缓存 ==========
// 结构：Map<odId, { userName, worldInfo, character, chatHistory, timestamp }>
...
function collectAndSendSyncData(params) {
  ...
  logSync('收集同步数据', {
    ...
    '角色性格长度': syncData.character.personality.length,
    '场景长度': syncData.character.scenario.length,
    '本地聊天条数': localChatHistory.length,
    '原始聊天条数': chat.length,
    '过滤掉的远程消息': chat.filter(m => m.extra?.isRemote).length,
  });
...
function injectRemoteContext(params) {
  ...
  logSync('注入远程上下文', {
    '世界书长度': remoteWorldInfo.length,
    '角色卡长度': remoteCharacter.length,
    '聊天历史条数': remoteChatHistory.length,
    '远程玩家数': remoteContextCache.size,
  });
}
...
function handleRemoteSyncContext(msg) {
  ...
  logSync('缓存远程上下文', {
    '玩家': senderName,
    '世界书Before长度': worldInfo?.before?.length || 0,
    '世界书After长度': worldInfo?.after?.length || 0,
    '角色描述长度': character?.description?.length || 0,
    '角色性格长度': character?.personality?.length || 0,
    '场景长度': character?.scenario?.length || 0,
...
function setupEventListeners() {
  ...
  eventSource.on(event_types.CHAT_CHANGED, function() {
    log('事件: 聊天切换');
    remoteStreamMap.clear();
    // 注意：不清除 remoteContextCache，保留远程上下文
    isGenerating = false;
    capturedHtml = null;
    capturedMessageId = null;
...
function debugEventInterceptor() {
  ...
  console.log('prepareOpenAIMessages已劫持:', !!window._prepareOpenAIMessagesHijacked);
