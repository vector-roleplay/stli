...
// ========== 世界书同步变量 ==========
let pendingWorldInfoEntries = null;
let pendingCharInfo = null;
...
// ========== 从缓存读取世界书（同步方式，备用）==========
function getWorldInfoFromCache() {
  const entries = [];
    ...
    if (cache && typeof cache.has === 'function') {
      for (const worldName of selected) {
        if (cache.has(worldName)) {
          const data = cache.get(worldName);
          if (data && data.entries) {
...
// ========== 判断内容是否匹配世界书（包含关系）==========
function isWorldInfoContent(packetContent, worldInfoEntries) {
  if (!packetContent || !worldInfoEntries || worldInfoEntries.length === 0) {
    return false;
  }
  
  for (const entry of worldInfoEntries) {
    if (entry.content && packetContent.includes(entry.content)) {
      return true;
    }
  }
  return false;
...
// ========== 判断内容是否匹配角色卡 ==========
function isCharacterContent(packetContent, charInfo) {
  if (!packetContent || !charInfo) {
    return false;
  }
  
  const fields = ['description', 'personality', 'scenario', 'first_mes', 'mes_example'];
  for (const field of fields) {
    if (charInfo[field] && charInfo[field].trim() && packetContent.includes(charInfo[field])) {
      return true;
    }
  }
  return false;
...
// ========== 从数据包提取需要同步的内容 ==========
function extractSyncContent(dataPacket, worldInfoEntries, charInfo) {
  const syncContent = [];
  ...
  for (const msg of dataPacket) {
    ...
    if (msg.role === 'system') {
      // 先检查是否是世界书内容
      if (isWorldInfoContent(content, worldInfoEntries)) {
        syncContent.push({
          type: 'worldInfo',
          content: content,
          role: 'system'
      ...
      // 再检查是否是角色卡内容
      if (isCharacterContent(content, charInfo)) {
        syncContent.push({
          type: 'character',
          content: content,
          role: 'system'
    ...
    // assistant消息通过流式同步，这里不处理
  }
  
  log('提取同步内容: ' + syncContent.length + ' 条');
  return syncContent;
}
...
function storeRemoteWorldInfo(senderId, senderName, syncContent, timestamp) {
  ...
  log('已缓存 ' + senderName + ' 的世界书，共 ' + syncContent.length + ' 条');
}

// ========== 合并远程世界书 ==========
function mergeRemoteWorldInfo() {
  ...
  // 生成联机模板
  merged.template = `[联机模式]
当前有多位玩家参与创作：
${playerNames.map(name => `- 玩家"${name}"的世界设定已附加在下方`).join('\n')}
请融合各方设定进行创作。`;
  
  log('合并远程世界书: ' + playerNames.length + ' 位玩家, ' + merged.contents.length + ' 条内容');
...
// 挂载到全局供调试
window.mpDebugWorldInfo = debugWorldInfo;

// ========================================
// 第3部分：发送按钮控制 + WebSocket连接
// ========================================
...
function setupEventListeners() {
  ...
  eventSource.on(event_types.MESSAGE_SENT, async function(messageId) {
    ...
    // 预加载世界书和角色卡（对照组）
    pendingWorldInfoEntries = await getAllWorldInfoEntries();
    pendingCharInfo = getCharacterInfo();
  ...
  eventSource.on(event_types.CHAT_COMPLETION_PROMPT_READY, function(data) {
    ...
    if (!currentRoom || !turnState.isMyTurn) return;
    if (!pendingWorldInfoEntries) return;
    
    log('事件: 数据包拦截');
    
    // 提取需要同步的内容
    const syncContent = extractSyncContent(data.chat, pendingWorldInfoEntries, pendingCharInfo);
