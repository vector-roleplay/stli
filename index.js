...
function addRemoteTag(messageId, labelText, type) {
  ...
  let tagStyle = '';
  if (type === 'ai') {
    tagStyle = 'background:#6366f1;color:#fff;';  // 紫色 - AI
  } else if (type === 'user') {
    tagStyle = 'background:#e94560;color:#fff;';  // 红色 - 用户
  } else {
    tagStyle = 'background:#888;color:#fff;';     // 灰色 - 默认
  }
...
function handleMessage(msg) {
  ...
  switch(msg.type) {
    ...
    case 'roomCreated':
      ...
      roomUsers = msg.users || [];
      chatMessages = [];
      roomJoinMessageIndex = getChat().length;  // 记录边界
