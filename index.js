...
function getActiveRegexScriptsForSync() {
  try {
    ...
    // 1. 获取全局正则
    const globalRegex = ctx.extensionSettings?.regex || [];
    ...
    log('获取到正则总数: ' + allScripts.length);
    
    if (allScripts.length === 0) {
      return [];
    }
    
    // 3. 筛选：只保留启用的、影响 AI_OUTPUT 的脚本
    const relevantScripts = allScripts.filter(script => {
      ...
      // placement 包含 2 (AI_OUTPUT)
      return script.placement.includes(REGEX_PLACEMENT.AI_OUTPUT);
    });
    
    log('筛选后正则数量: ' + relevantScripts.length);
 ...
 * @param {Array} regexScripts 正则脚本数组
...
/**
 * 处理 trimStrings - 从文本中移除指定字符串
 * @param {string} text 原始文本
 * @param {Array} trimStrings 需要移除的字符串数组
 * @returns {string} 处理后的文本
 */
function applyTrimStrings(text, trimStrings) {
  if (!text || !trimStrings || !Array.isArray(trimStrings) || trimStrings.length === 0) {
    return text;
  }
  
  let result = text;
  for (const trimStr of trimStrings) {
    if (trimStr && typeof trimStr === 'string') {
      // 全局替换
...
function executeRegexScript(script, text) {
  ...
  try {
    ...
    if (!regex) {
      log('正则解析失败，跳过: ' + script.scriptName);
      return text;
    }
    ...
    const result = text.replace(regex, function(match, ...args) {
      // args 包含: [捕获组1, 捕获组2, ..., offset, 原始字符串, 命名捕获组对象(如果有)]
      ...
      // 替换 $1-$99 为对应的捕获组
      replacement = replacement.replace(/\$(\d{1,2})/g, function(m, groupNum) {
        const index = parseInt(groupNum, 10);
        // args 中捕获组从索引0开始，$1 对应 args[0]
        if (index > 0 && index <= args.length) {
          const captured = args[index - 1];
          if (captured !== undefined && captured !== null) {
            // 应用 trimStrings 到捕获组
            return applyTrimStrings(String(captured), trimStrings);
          }
        }
        return '';
      });
      ...
      // 替换 $<name> 命名捕获组
      replacement = replacement.replace(/\$<([^>]+)>/g, function(m, groupName) {
        // 命名捕获组在 args 的最后一个对象中（如果存在）
        const lastArg = args[args.length - 1];
        if (lastArg && typeof lastArg === 'object' && lastArg[groupName] !== undefined) {
          return applyTrimStrings(String(lastArg[groupName]), trimStrings);
        }
        return '';
      });
    ...
    log('正则执行失败: ' + (script.scriptName || '未知') + ' - ' + e.message);
    return text;
  }
}
...
function applyRemoteRegex(text, senderId) {
  ...
  log('应用远程正则: ' + cached.userName + ', ' + cached.regexScripts.length + ' 条规则');
...
function injectRemoteWorldInfoViaExtensionPrompt() {
  ...
  // 通过 getContext 获取 setExtensionPrompt
  const ctx = getContext();
...
function handleRemoteAiComplete(msg) {
  ...
  log('远程AI完成，原始长度: ' + (msg.content ? msg.content.length : 0) + ', 格式化后: ' + (formattedContent ? formattedContent.length : 0));
...
function debugRegex() {
  ...
  const ctx = getContext();
  const globalRegex = ctx.extensionSettings?.regex || [];
  console.log('全局正则数量:', globalRegex.length);
  
  const charId = ctx.characterId;
  const char = ctx.characters && ctx.characters[charId];
  const scopedRegex = char?.data?.extensions?.regex_scripts || [];
  console.log('角色绑定正则数量:', scopedRegex.length);
  
  // 显示前3条正则的详细信息
  const allLocal = [...globalRegex, ...scopedRegex];
  console.log('本地正则总数:', allLocal.length);
  
  allLocal.slice(0, 3).forEach((script, i) => {
    console.log(`[本地 ${i}] ${script.scriptName}`);
    console.log(`  - findRegex: ${script.findRegex}`);
    console.log(`  - replaceString: ${script.replaceString}`);
    console.log(`  - placement: ${JSON.stringify(script.placement)}`);
    console.log(`  - disabled: ${script.disabled}`);
  });
  
  // 远程正则缓存
  console.log('\n远程正则缓存:', remoteRegexCache.size, '个玩家');
    ...
    remoteRegexCache.forEach(function(data, odId) {
      ...
      if (data.regexScripts) {
        data.regexScripts.forEach(function(script, idx) {
          console.log(`  [${idx}] ${script.scriptName}: ${script.findRegex} -> ${script.replaceString}`);
        });
      }
    });
  }
...
function testRegex(text, senderId) {
  console.log('===== 测试正则应用 =====');
...
function testLocalRegex(text) {
  ...
  const scripts = getActiveRegexScriptsForSync();
  console.log('获取到正则数量:', scripts.length);
...
window.mpDebug = {
  ...
  // 手动执行
  getActiveRegex: getActiveRegexScriptsForSync,
  applyRemoteRegex: applyRemoteRegex,
  executeRegexScript: executeRegexScript,
